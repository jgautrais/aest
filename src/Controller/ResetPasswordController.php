...
class ResetPasswordController extends AbstractController
{
    ...
    public function request(Request $request, MailerInterface $mailer): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            if (!is_string($form->get('email')->getData())) {
    ...
    private function processSendingPasswordResetEmail(string $emailFormData, MailerInterface $mailer): RedirectResponse
    {
        ...
        if (!is_string($user->getEmail()) || !is_string($this->getParameter('mailer_from'))) {
            throw new Exception('Email is not of type string');
        }

        $email = (new TemplatedEmail())
            ->from(new Address($this->getParameter('mailer_from'), 'Aest'))
            ->to($user->getEmail())
            ->subject('Aest - Your password reset request')
